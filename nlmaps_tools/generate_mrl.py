...
def render_nwr(nwr_features, escape=True):
    ...
    for feat in nwr_features:
        if feat[0] in ["or", "and"] and isinstance(feat[1], (list, tuple)):
            ...
            parts.append(
                "keyval({},{})".format(quote(feat[0], escape), quote(feat[1], escape))
            )
        else:
...
def open_paren_after_functor(nested_tuple, escape=True):
    ...
    for elm in nested_tuple:
        if isinstance(elm, (str, Symbol)):
            ...
            functor = elm[0]
            parts.append(
                "{}({})".format(functor, open_paren_after_functor(elm[1:], escape))
            )
        else:
...
def test():
    ...
    features = {
        "sub": [
            {
                "area": "Edinburgh",
                "target_nwr": [("name", "Palace of Holyroodhouse")],
                "query_type": "in_query",
                "qtype": (Symbol("latlong"),),
            },
            {
                "area": "Edinburgh",
                "target_nwr": [("name", "Camera Obscura")],
                "query_type": "in_query",
                "qtype": (Symbol("latlong"),),
            },
        ],
