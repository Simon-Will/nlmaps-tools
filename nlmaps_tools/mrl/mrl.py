...
class MRL:
    ...
    def count_arguments(self, s):
        ...
        i = 0
        while i < len(s) and (
            (not args_found and num_brackets == 0) or (args_found and num_brackets > 0)
        ):
            c = s[i : i + 1]
            if c == "(":
                args_found = True
...
class NLmaps(MRL):
    ...
    def preprocess_mrl(self, mrl):
        ...
        mrl = re.sub(
            ...
            r"and(\g<1>@s','\g<2>@s)",
            mrl,
        )  # for when a and() surrounds two end values
        mrl = re.sub(
            r"\(' *([^\(\)]+?) *'\)", r"(\g<1>@s)", mrl
        )  # a bracket ( or ) is not allowed withing any key or value
        mrl = re.sub(
            r"([,\)\(])or\(([^\(\)]+?)','([^\(\)]+?)@s\)",
            r"\g<1>or(\g<2>@s','\g<3>@s)",
            mrl,
        )  # for when a or() surrounds two values
        mrl = re.sub(r"\s+", " ", mrl)
        mrl = re.sub(r"'", "", mrl)
        mrl = mrl.strip()
    ...
    def linearise(self, mrl):
        ...
        for element in just_words.split(" "):
            ...
            if element.endswith("@s"):
                lin.append(element)
                continue
            shortened_string = self.delete_first_n_occurences(
                mrl, element, seen_string_x_times[element]
            )
    ...
    def check_MRL_tree(self, mrl, cfg):
        ...
        try:
            ...
            mrl = re.sub(
                r"keyval\( '([^\(\)]+?)' , '[^\(\)]+?' ",
                r"keyval( '\g<1>' , 'valvariable' ",
                mrl,
            )
            mrl = re.sub(
                r"keyval\( '([^\(\)]+?)' , or\( '[^\(\)]+?' , '[^\(\)]+?' ",
                r"keyval( '$1' , or( 'valvariable' , 'valvariable' ",
                mrl,
            )
            mrl = re.sub(
                r"keyval\( '([^\(\)]+?)' , and\( '[^\(\)]+?' , '[^\(\)]+?' ",
                r"keyval( '$1' , and( 'valvariable' , 'valvariable' ",
                mrl,
            )
            ...
            if m:
                ...
                for digit in m.group(1):
                    new_number = new_number + digit + " "
                    mrl = re.sub(
                        r"maxdist\( (.*?)\)", r"maxdist( " + new_number + ")", mrl
                    )
            ...
            ini_file.close()
            args = [
                "%s/decoder/cdec" % self.cdec,
                "-c",
                "%s/cdec_validate.ini" % (temp_dir),
            ]
            ...
            p = subprocess.Popen(
                args, stdin=infile, stdout=nullfile, stderr=subprocess.PIPE
            )
...
MRLS = collections.OrderedDict([("", MRL), ("nlmaps", NLmaps)])
