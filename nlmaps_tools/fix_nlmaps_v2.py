...
SUBSTITUTIONS = (
    (
        r"keyval\('cuisine','vegetarian'\)",
        r"keyval('diet:vegetarian',or('only','yes'))",
    ),
    (
        r"keyval\('(historic|building)','church'\)",
        r"keyval('amenity','place_of_worship'),keyval('religion','christian')",
    ),
    (r"keyval\('building','stadium'\)", r"keyval('leisure','stadium')"),
    (
        r"keyval\(('amenity','public_building'|'building','public')\)",
        r"keyval('building',or('civic','public'))",
    ),
    ...
    (
        r"keyval\('highway','(primary_link|primary)'\)",
        r"keyval('highway',or('primary','primary_link'))",
    ),
    (
        r"keyval\('highway','(secondary_link|secondary)'\)",
        r"keyval('highway',or('secondary','secondary_link'))",
    ),
    (
        r"keyval\('highway','(trunk_link|trunk)'\)",
        r"keyval('highway',or('trunk','trunk_link'))",
    ),
    ...
    (
        r"keyval\('shop','food'\)",
        r"keyval('shop',or('convenience','deli','supermarket'))",
    ),
    ...
    (
        r"keyval\(('landuse','farm'|'place','farm')\)",
        r"or(keyval('landuse','farmyard'),keyval('place','farm'))",
    ),
    (
        r"keyval\(('landuse','wood'|'landuse','forest'|'natural','wood')\)",
        r"or(keyval('landuse','forest'),keyval('natural','wood'))",
    ),
    (
        r"keyval\('amenity','nursing_home'\)",
        r"or(keyval('amenity','nursing_home'),keyval('social_facility','nursing_home'))",
    ),
    (
        r"keyval\('(amenity|cuisine)','ice_cream'\)",
        r"or(keyval('amenity','ice_cream'),keyval('cuisine','ice_cream'))",
    ),
    ...
    (
        r"or\(keyval\('organic','only'\),keyval\('organic','yes'\)\)",
        r"keyval('organic',or('only','yes'))",
    ),
...
def fix_bar_pub(mrl, en):
    if re.search(r"\bbars?\b", en, flags=re.IGNORECASE):
        mrl = re.sub(r"keyval\('amenity','pub'\)", "keyval('amenity','bar')", mrl)
    elif re.search(r"\bpubs?\b", en, flags=re.IGNORECASE):
        mrl = re.sub(r"keyval\('amenity','bar'\)", "keyval('amenity','pub')", mrl)
    return mrl


def fix_drinking_water(mrl, en):
    if re.search(r"\bwaters?\b", en, flags=re.IGNORECASE) and not re.search(
        r"drink", en, flags=re.IGNORECASE
    ):
        mrl = re.sub(
...
def main(mrl_infile, en_infile, mrl_outfile, en_outfile, quiet=False):
    ...
    modified = 0
    total = 0
    with open(mrl_infile) as mrl_inf, open(en_infile) as en_inf, open(
        mrl_outfile, "w"
    ) as mrl_outf, open(en_outfile, "w") as en_outf:
...
def parse_args():
    ...
    parser.add_argument("mrl_infile", help="NLMaps v2 MRL file")
    parser.add_argument(
        "en_infile", help="NLMaps v2 english NL file corresponding to mrl_infile"
    )
    parser.add_argument("mrl_outfile", help="Output file for writing the fixed mrls")
    parser.add_argument(
        "en_outfile", help="Output file for writing the corresponding english queries"
    )
    parser.add_argument(
        "--quiet",
        "-q",
